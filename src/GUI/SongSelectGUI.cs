...
namespace Instruments.GUI
{
	public class SongSelectGUI : GuiDialog
	{
		...
		private List<FileTree.Node> _shownStackList;
		...
		public SongSelectGUI(ICoreClientAPI capi, string directory, Action<string> bandChange = null, string bandName = "") : base(capi)
		{
			...
			_shownStackList = new List<FileTree.Node>();
		...
		private void SetupDialog(string bandName)
		{
			...
			SingleComposer = capi.Gui.CreateCompo("SongSelectDialog", dialogBounds)
				...
				.BeginClip(clipBounds)
					...
					.AddFlatList(stackListBounds, ButtonPressed, Unsafe.As<List<IFlatListItem>>(_shownStackList), "stacklist")
		...
		private void ButtonPressed(int index)
		{
			...
			FileTree.Node node = _shownStackList[index];
			if (node.IsDirectory)
		...
		{
			_shownStackList.Clear();
			_fileTree.GetNodes(_shownStackList, true);
